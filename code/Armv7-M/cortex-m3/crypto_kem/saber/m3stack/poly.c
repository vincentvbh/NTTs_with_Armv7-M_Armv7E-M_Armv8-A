...
void InnerProdDecNTT_stack(uint8_t m[SABER_KEYBYTES], const uint8_t ciphertext[SABER_BYTES_CCA_DEC], const uint8_t sk[SABER_INDCPA_SECRETKEYBYTES]){
    ...
    int16_t poly_NTT[SABER_N];
    int16_t buff_NTT[SABER_N];
    ...
    for (i = 0; i < SABER_L; i++) {
        ...
        if(i == 0){
            NTT_mul1((int16_t*)acc_NTT, buff_NTT, poly_NTT);
        }else{
            NTT_mul_acc1((int16_t*)acc_NTT, buff_NTT, poly_NTT);
        }
        ...
        if(i == 0){
            NTT_mul2((int16_t*)(acc_NTT + SABER_N / 2), buff_NTT, poly_NTT);
        }else{
            NTT_mul_acc2((int16_t*)(acc_NTT + SABER_N / 2), buff_NTT, poly_NTT);
        }
...
void MatrixVectorMulKeyPairNTT_D(uint8_t pk[SABER_INDCPA_PUBLICKEYBYTES], uint8_t sk[SABER_INDCPA_SECRETKEYBYTES]){
    ...
    int16_t *poly_NTT = (int16_t*)poly;
    int16_t *s_NTT = (int16_t*)s;
    int16_t *buff_NTT = (int16_t*)buff;
