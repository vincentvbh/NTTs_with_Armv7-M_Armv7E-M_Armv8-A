...
extern void __asm_negacyclic_ntt(int16_t *des, const int32_t *root_table, int32_t Qprime, int32_t Q, uint16_t *src, int32_t RmodQ);
extern void __asm_base_mul(int16_t *des, const int32_t *mul_Rmod_table, int32_t Qprime, int32_t Q, int16_t *src1, int16_t *src2);
extern void __asm_base_mul_acc(int16_t *des, const int32_t *mul_Rmod_table, int32_t Qprime, int32_t Q, int16_t *src1, int16_t *src2, int32_t RmodQ);
extern void __asm_intt(int16_t *des, const int32_t *inv_root_table, int32_t Qprime, int32_t Q, int32_t RmodQ);
extern void __asm_solv_CRT(uint16_t *des, int16_t *src2, int16_t *src1, const int32_t *_final_constants);

#define NTT_forward1(out, in) __asm_negacyclic_ntt(out, streamlined_CT_negacyclic_table_Q1, Q1prime, Q1, in, Q1bar)
#define NTT_forward2(out, in) __asm_negacyclic_ntt(out, streamlined_CT_negacyclic_table_Q2, Q2prime, Q2, in, Q2bar)

#define NTT_forward(out, in) { \
        NTT_forward1((int16_t*)(out + 0 * SABER_N / 2), in); \
        NTT_forward2((int16_t*)(out + 1 * SABER_N / 2), in); \
    }
...
#define NTT_mul(out, in1, in2) { \
        NTT_mul1((int16_t*)(out + 0 * SABER_N / 2), (int16_t*)(in1 + 0 * SABER_N / 2), (int16_t*)(in2 + 0 * SABER_N / 2)); \
        NTT_mul2((int16_t*)(out + 1 * SABER_N / 2), (int16_t*)(in1 + 1 * SABER_N / 2), (int16_t*)(in2 + 1 * SABER_N / 2)); \
    }

#define NTT_mul_acc1(out, in1, in2) __asm_base_mul_acc(out, mul_Rmod_table_Q1, Q1prime, Q1, in1, in2, RmodQ1)
#define NTT_mul_acc2(out, in1, in2) __asm_base_mul_acc(out, mul_Rmod_table_Q2, Q2prime, Q2, in1, in2, RmodQ2)

#define NTT_mul_acc(out, in1, in2) { \
        NTT_mul_acc1((int16_t*)(out + 0 * SABER_N / 2), (int16_t*)(in1 + 0 * SABER_N / 2), (int16_t*)(in2 + 0 * SABER_N / 2)); \
        NTT_mul_acc2((int16_t*)(out + 1 * SABER_N / 2), (int16_t*)(in1 + 1 * SABER_N / 2), (int16_t*)(in2 + 1 * SABER_N / 2)); \
    }

#define NTT_inv1(in) __asm_intt(in, streamlined_inv_CT_negacyclic_table_Q1, Q1prime, Q1, Q1bar)
#define NTT_inv2(in) __asm_intt(in, streamlined_inv_CT_negacyclic_table_Q2, Q2prime, Q2, Q2bar)
#define solv_CRT(out, in1, in2) __asm_solv_CRT(out, in2, in1, final_constants)

#define NTT_inv(out, in) { \
        NTT_inv1((int16_t*)(in + 0 * SABER_N / 2)); \
        NTT_inv2((int16_t*)(in + 1 * SABER_N / 2)); \
        solv_CRT(out, (int16_t*)(in + 0 * SABER_N / 2), (int16_t*)(in + 1 * SABER_N / 2)); \
    }
