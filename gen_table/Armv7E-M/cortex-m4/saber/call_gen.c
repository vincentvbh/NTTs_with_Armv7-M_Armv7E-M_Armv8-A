...
static const int16_t streamlined_CT_negacyclic_table_Q1[(NTT_N - 1) + (1 << 0) + (1 << 3)] = {
...
// ================
    ...
    for(int i = 0; i < (NTT_N >> 1); i++){
        assert(mul_Rmod_table_Q1Q2[i] == twiddle_int32[i]);
    }
...
// ================
    ...
    for(int i = 0; i < (NTT_N >> 1); i++){
        assert(mul_Rmod_table_Q1[i] == twiddle_int16[i]);
    }
